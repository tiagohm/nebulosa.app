...
export class TppaTask {
	...
	private async cameraCaptured(event: CameraCaptureEvent, path?: string) {
		if (path && !this.stopped) {
			...
			// Capture next image
			if (!this.stopped && (await this.captureReleased())) this.start()
	...
	start() {
		...
		this.capture = this.tppa.cameraHandler.capture(this.tppa.cameraHandler.coordinator, this.camera, this.request.capture, (event, path) => {
