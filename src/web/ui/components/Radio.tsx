	...
	return (
		<label className={cn(styles.base(), disabled ? 'cursor-not-allowed opacity-40' : readOnly ? 'cursor-default opacity-90 pointer-events-none' : 'cursor-pointer', className, classNames?.base)}>
			<input
				...
				type="radio"
			/>
			<span className={cn(styles.control(), checked ? 'border-(--color-variant)' : 'border-none', !disabled && !readOnly && (checked ? 'hover:border-(--color-variant)/90 active:border-(--color-variant)/80' : 'hover:bg-neutral-800 active:bg-neutral-700'), classNames?.control)}>
				<span className={cn(styles.indicator(), checked ? 'scale-100 opacity-100 bg-(--color-variant)' : 'scale-75 opacity-0 bg-transparent', classNames?.indicator)} />
			</span>
