...
export function Paginator({ page, count, onPrev, onNext, loading = false, readOnly = true, className, ...props }: PaginatorProps) {
	return (
		<div {...props} className={cn('flex flex-row items-center justify-center gap-3', className)}>
			<IconButton color="secondary" disabled={page <= 1 || loading} icon={Icons.ChevronLeft} onClick={onPrev} tooltipContent="Prev page" />
			...
			<IconButton color="secondary" disabled={count < 4 || loading} icon={Icons.ChevronRight} onClick={onNext} tooltipContent="Next page" />
		</div>
	)
}
