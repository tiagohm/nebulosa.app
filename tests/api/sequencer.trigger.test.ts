...
function autofocus(triggers: Partial<SequencerAutofocus['triggers']> = {}): Omit<SequencerAutofocus, 'enabled'> {
	const merged = { onStart: false, onFilterChange: false, afterMeridianFlip: false, everyFrames: 0, everyTime: 0, temperatureChange: 0, minimumTimeBetweenRuns: 0, ...triggers }
	return { triggers: merged } as unknown as Omit<SequencerAutofocus, 'enabled'>
}

function dither(overrides: Partial<SequencerDitherTrigger> = {}): SequencerDitherTrigger {
	return { beforeFirstFrame: false, afterFilterChange: false, everyFrames: 0, everyTime: 0, ...overrides } as unknown as SequencerDitherTrigger
}
...
describe('sequencer trigger evaluator', () => {
	...
	test('keeps the autofocus owed by a flip pending until a run focused', () => {
		const policies = { meridianFlip: meridianFlip(), autofocus: autofocus({ afterMeridianFlip: true }), dither: dither({ everyFrames: 1 }) }
		...
		expect(dithered.pendingAutofocus).toBe('afterMeridianFlip')
		expect(kinds(policies, dithered, second)).toEqual(['autofocus:afterMeridianFlip'])
	...
	test('owes no post-flip focus to a definition whose autofocus does not ask for one', () => {
		...
		expect(sequencerTriggerPending(policies, anchors(), evaluateSequencerTriggers(policies, anchors(), observed)).pendingAutofocus).toBeUndefined()
	})

	test('keeps a post-flip focus owed while the minimum time between runs suppresses it', () => {
		const policies = { meridianFlip: meridianFlip(), autofocus: autofocus({ afterMeridianFlip: true, minimumTimeBetweenRuns: 600 }) }
		const focused = sequencerAnchorAdvanced(anchors(), 'autofocus', observation())
		const crossing = observation({ instant: START + 60_000, hourAngle: 0.02, pierSide: 'WEST', preFlipPierSide: 'WEST' })

		expect(kinds(policies, focused, crossing)).toEqual(['meridianFlip:hourAngle'])

		const state = sequencerTriggerPending(policies, focused, evaluateSequencerTriggers(policies, focused, crossing))

		expect(state.pendingAutofocus).toBe('afterMeridianFlip')
		expect(kinds(policies, state, observation({ instant: START + 700_000 }))).toEqual(['autofocus:afterMeridianFlip'])
	})
	...
	test('drops an owed autofocus whose trigger is no longer enabled', () => {
		...
		expect(kinds({ autofocus: autofocus({ everyFrames: 5 }) }, state, observed)).toEqual([])
	})
