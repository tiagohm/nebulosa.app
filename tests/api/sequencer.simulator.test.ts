...
function lightFiles(files: readonly string[]) {
	return files.filter((path) => path.split(sep).includes('LIGHT') && path.endsWith('.fits'))
}
...
describe('canonical night', () => {
	test('A.01 full LRGB night', async () => {
		...
		const lights = lightFiles(night.files)
	...
	test('A.02 snapshot during the first autofocus', async () => {
		let duringAutofocus: { readonly state: string; readonly exposure?: unknown; readonly type?: string } | undefined
		const night = await runNight({
			control: async (api) => {
				const snapshot = await api.waitUntil((current) => current.foreground?.type === 'trigger.autofocus')

				duringAutofocus = { state: snapshot.state, exposure: snapshot.capture.exposure, type: snapshot.foreground?.type }
			},
		})

		nights.push(night)

		expect(duringAutofocus).toEqual({ state: 'running', exposure: undefined, type: 'trigger.autofocus' })
		expect(night.session.state).toBe('completed')
	...
	test('A.03 second run of the same definition', async () => {
		...
		const lights = lightFiles(second.files)
	...
	test('A.05 without a wheel', async () => {
		const night = await runNight({
			patch: {
				...
				capture: { frames: [frame('lum', { count: 4 }, { exposureTime: 2 })] },
			},
		})
	...
	test('A.09 unguided field rig', async () => {
		const night = await runNight({
			patch: {
				...
				capture: { frames: [frame('lum', { count: 8 }, { exposureTime: 0.5 })] },
				startup: { enabled: false },
				shutdown: { enabled: false },
			},
			sim: {
	...
	test('A.10 narrowband Ha OIII SII night', async () => {
		const night = await runNight({
			patch: {
				capture: {
					frames: [frame('ha', { count: 2 }, { exposureTime: 2, filter: { type: 'name', name: 'Ha' } }), frame('o3', { count: 2 }, { exposureTime: 2, filter: { type: 'name', name: 'O3' } }), frame('s2', { count: 2 }, { exposureTime: 2, filter: { type: 'name', name: 'S2' } })],
				},
				autofocus: {
	...
	test('A.11 lucky imaging planetary night', async () => {
		const night = await runNight({
			patch: {
				...
				capture: { delay: 0, frames: [frame('lum', { count: 40 }, { exposureTime: 0.5, filter: { type: 'name', name: 'L' } })] },
			},
		})
...
describe('admission', () => {
	...
	test('B.04 a disconnected guider refuses start before any command', async () => {
		...
		const created = process.handler.createSession(
			process.definition(observatory, {
				...
				meridianFlip: { enabled: false },
				target: { center: { enabled: false } },
				capture: { delay: 0, frames: [frame('lum', { count: 2 }, { exposureTime: 0.5, filter: { type: 'name', name: 'L' } })] },
	...
	test('B.11 a stop mid-exposure releases only after the cleanups', async () => {
		...
		expect(lightFiles(night.files)).toHaveLength(1)
...
describe('startup', () => {
	test('D.01 startup actions run in canonical order', async () => {
		const night = await runNight()
	...
	test('D.06 a required unpark failure still runs later optional actions when continueOnFailure is true', async () => {
		const night = await runNight({
			patch: { startup: { continueOnFailure: true } },
	...
	test('D.11 an enabled guider with the startup pipeline disarmed is refused', async () => {
		...
		expect(started.preflight?.diagnostics).toEqual([{ path: 'guiding.enabled', message: 'the guiding block declares the guider the capture runs under, and the startup pipeline that starts it is disabled, so every frame would be captured unguided' }])
	...
	test('D.14 a disabled openCover is skipped and light opens the cover', async () => {
		const night = await runNight({
			patch: { cover: { openOnStartup: false } },
		})
	...
	test('D.23 a disabled required unpark is dropped and does not fail as notRun', async () => {
		const night = await runNight({
			patch: { mount: { unparkOnStartup: false } },
		})
	...
	test('D.25 initial slew and center run under the guiding interlock', async () => {
		const night = await runNight({
			patch: {
				...
				dither: { enabled: false },
				meridianFlip: { enabled: false },
				capture: { delay: 0, frames: [frame('lum', { count: 1 }, { filter: { type: 'name', name: 'L' } })] },
			},
		})
	...
	test('D.31 a disabled mount feature omits unpark even when the flag is on', async () => {
		const night = await runNight({
			patch: { mount: { enabled: false, unparkOnStartup: true } },
		})
	...
	test('E.27 the slew waits the target settle before the next action and not between lights', async () => {
		const night = await runNight({
			patch: {
				...
				meridianFlip: { enabled: false },
				rotator: { enabled: false },
				capture: { delay: 0, frames: [frame('lum', { count: 2 }, { exposureTime: 2 })] },
			},
		})
