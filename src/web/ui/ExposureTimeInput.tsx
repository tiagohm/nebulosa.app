...
export interface ExposureTimeInputProps extends Omit<NumberInputProps, 'size' | 'minValue' | 'maxValue'> {
...
export function ExposureTimeInput({ className, value, onValueChange, unit, onUnitChange, minimumInMicrosseconds, maximumInMicrosseconds }: ExposureTimeInputProps) {
	...
	return (
		<div className={`flex flex-row gap-1 items-center ${className}`}>
			<NumberInput endContent={<ExposureTimeUnitDropdown color='secondary' onValueChange={onUnitChange} value={unit} />} label='Exposure Time' maxValue={maxValue} minValue={minValue} onValueChange={onValueChange} size='sm' value={value} />
		</div>
	)
}
