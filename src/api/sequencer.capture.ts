...
function frameCapture(group: SequencerPlanFrameGroup, staged: string, devices: SequencerCaptureDevices): CameraCaptureStart {
	...
	return {
		...
		subframe: camera.subframe.enabled,
		x: camera.subframe.x,
		y: camera.subframe.y,
		width: camera.subframe.width,
		height: camera.subframe.height,
...
async function runCapture(services: SequencerCaptureServices, context: SequencerActionContext, configuration: SequencerCapture): Promise<SequencerActionResult<SequencerCaptureOutcome>> {
	...
	context.progress({ fraction: 0, detail: `exposing ${configuration.group.name} for ${configuration.group.exposureTime}s` })
