...
export function ImageViewer() {
	...
	const { info, crosshair, transformation } = useSnapshot(viewer.state)
		...
		<ContextMenu>
			<ContextMenuTrigger className='block' onContextMenu={(e) => console.info(e, e.nativeEvent.offsetX)}>
				<div className='inline-block absolute wrapper' style={{ zIndex: image.index }}>
					<img ref={ref} id={image.key} onLoad={(e) => viewer.attach(e.currentTarget)} className='image select-none shadow-md' onPointerUp={(e) => viewer.bringToFront(e.currentTarget)} />
					{crosshair && <Crosshair />}
				</div>
			</ContextMenuTrigger>
			<ContextMenuContent>
