...
const SOLVER_PROPERTIES = ['ASTAP', 'ASTROMETRY_NET', 'NOVA_ASTROMETRY_NET'] as const

const state = proxy(structuredClone(DEFAULT_SETTINGS_STATE))
populateProxy(state.solver, 'settings.solver', SOLVER_PROPERTIES)
subscribeProxy(state.solver, 'settings.solver', SOLVER_PROPERTIES)
