...
export interface PlateSolveStartPopoverProps extends Pick<PlateSolveStart, PlateSolveStartPopoverKey> {
	readonly showAdvancedOnly?: boolean
	readonly onValueChange?: (key: PlateSolveStartPopoverKey, value: number) => void
}

export function PlateSolveStartPopover({ radius, focalLength, pixelSize, downsample, timeout, showAdvancedOnly = false, onValueChange }: PlateSolveStartPopoverProps) {
	return (
		<Popover className='max-w-110' placement='bottom' showArrow>
			...
			<PopoverContent>
				<div className='grid grid-cols-12 gap-2 p-4'>
					...
					{!showAdvancedOnly && <NumberInput className='col-span-3' formatOptions={DECIMAL_NUMBER_FORMAT} label='Radius (°)' maxValue={360} minValue={0} onValueChange={(value) => onValueChange?.('radius', value)} size='sm' step={0.1} value={radius ?? 4} />}
					{!showAdvancedOnly && <NumberInput className='col-span-5' formatOptions={INTEGER_NUMBER_FORMAT} label='Focal Length (mm)' maxValue={100000} minValue={0} onValueChange={(value) => onValueChange?.('focalLength', value)} size='sm' value={focalLength} />}
					{!showAdvancedOnly && <NumberInput className='col-span-4' formatOptions={DECIMAL_NUMBER_FORMAT} label='Pixel size (µm)' maxValue={1000} minValue={0} onValueChange={(value) => onValueChange?.('pixelSize', value)} size='sm' step={0.01} value={pixelSize} />}
