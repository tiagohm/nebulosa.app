	...
	return (
		<Modal footer={Footer} header='Connection' id='connection' maxWidth='240px' onHide={connection.hide}>
			<div className='mt-0 grid grid-cols-12 gap-2'>
				...
				<NumberInput className='col-span-5' formatOptions={INTEGER_NUMBER_FORMAT} label='Port' maxValue={65535} minValue={80} onValueChange={(value) => connection.update('port', value)} placeholder={edited.type === 'INDI' ? '7624' : '32323'} size='sm' value={edited.port} />
				<ClientTypeSelect className='col-span-5' onValueChange={(value) => connection.update('type', value)} value={edited.type} />
				<Checkbox className='col-span-7' isDisabled={edited.type !== 'ALPACA'} isSelected={edited.secured} onValueChange={(value) => connection.update('secured', value)}>
					Secured
				</Checkbox>
			</div>
		</Modal>
