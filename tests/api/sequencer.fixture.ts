...
export function canonical(): Sequencer {
	return {
		...
		target: {
			...
			goto: { enabled: true, tolerance: 0.001, arrivalTolerance: 0.0005, timeout: 300, settle: 5, retry: retry() },
			center: {
				enabled: true,
		...
		dither: { enabled: true, amount: 3, raOnly: false, beforeFirstFrame: false, afterFilterChange: false, everyFrames: 1, everyTime: 0, settle: { tolerance: 1.5, time: 10, timeout: 120 }, retry: retry(), onFailure: 'continue' },
		autofocus: {
			enabled: true,
		...
		cooling: { enabled: true, temperature: -10, tolerance: 1, ramp: 2, waitForTarget: true, timeout: 900, maintainDuringPause: true, maintainDuringSuspension: true, warmTemperature: 15, warmRamp: 2, turnCoolerOffAfterWarm: true },
		...
		calibration: {
			dark: { enabled: false, order: 'afterLights', count: 20, retry: retry(), everyFrames: 0, temperatureChange: 0, matchLightFrames: true, exposureTimes: [], moveDarkFilter: false },
			...
			darkFlat: { enabled: false, order: 'afterLights', count: 20, retry: retry(), matchFlatFrames: true, exposureTimes: [], moveDarkFilter: false },
		},
		...
		quality: { enabled: false, starDetection: { type: 'nebulosa', timeout: 30, minimumSNR: 10, maximumStars: 500 }, evaluateEveryFrames: 1, rejectFrame: false, repeatRejectedFrame: false, maximumRepeatedFrames: 0, pauseAfterConsecutiveRejectedFrames: 0 },
		execution: {
			start: { type: 'manual' },
