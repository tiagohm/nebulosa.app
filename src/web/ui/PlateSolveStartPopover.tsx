...
export function PlateSolveStartPopover({ type, radius, focalLength, pixelSize, disabled = false, onValueChange }: PlateSolveStartPopoverProps) {
	...
	const { apiKey, apiUrl, downsample, executable, timeout } = useSnapshot(settings.state.solver[type])
	const isNovaAstrometryNet = type === 'NOVA_ASTROMETRY_NET'
	...
	return (
		<Popover className="max-w-130" disabled={disabled} trigger={<IconButton disabled={disabled} icon={Icons.Cog} size="sm" tooltipContent="Plate solve options" />}>
			<div className="grid grid-cols-12 gap-2 p-4">
				...
				{!isNovaAstrometryNet && <TextInput className="col-span-full" disabled={disabled} label="Executable" onValueChange={(value) => settings.updateSolver(type, 'executable', value)} value={executable} />}
				{isNovaAstrometryNet && <TextInput className="col-span-8" disabled={disabled} label="API URL" onValueChange={(value) => settings.updateSolver(type, 'apiUrl', value)} placeholder="https://nova.astrometry.net" value={apiUrl} />}
				{isNovaAstrometryNet && <TextInput autoComplete="off" className="col-span-4" disabled={disabled} label="API Key" onValueChange={(value) => settings.updateSolver(type, 'apiKey', value)} placeholder="XXXXXXXX" value={apiKey} />}
				...
				<NumberInput className="col-span-6" disabled={disabled} label="Downsample factor" maxValue={4} minValue={0} onValueChange={(value) => settings.updateSolver(type, 'downsample', value)} value={downsample} />
				<NumberInput className="col-span-6" disabled={disabled} label="Timeout (ms)" maxValue={600000} minValue={0} onValueChange={(value) => settings.updateSolver(type, 'timeout', value)} value={timeout} />
			</div>
		</Popover>
	)
}
