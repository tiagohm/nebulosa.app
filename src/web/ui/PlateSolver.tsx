...
export function PlateSolver() {
	...
	return (
		<Modal {...modal.props} classNames={{ base: 'max-w-[410px] max-h-[90vh]', wrapper: 'pointer-events-none' }} onOpenChange={(value) => (viewer.state.plateSolver.showModal = value)}>
			<ModalContent>
				{() => (
					<>
						...
						<ModalFooter {...modal.moveProps}>
							<Button color='success' isLoading={plateSolver.loading} onPointerUp={() => viewer.detectStars()} startContent={<Lucide.Sigma />} variant='flat'>
								Solve
							</Button>
						</ModalFooter>
					</>
